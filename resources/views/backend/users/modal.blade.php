...
<form action="{{$url}}" method="post" data-form="ajax-form" data-modal="#ajax_model" data-datatable="#users_datatable">
    ...
    <div class="row">
        ...
        <div class="form-group col-lg-6">
            ...
            <input type="tel" class="form-control" name="phone" id="phone" value="{{$isEdit ? $user->phone : ''}}">
        </div>
        ...
        <div class="form-group col-lg-6">
            ...
            <select class="form-control select2 form-select form-select-modal" name="role" id="role">
                ...
                @foreach ($roles as $role)
                    <option value="{{ $role->name }}" @if ($isEdit && in_array($role->name, $user->roles->pluck('name')->toArray())) selected @endif>{{ $role->title }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group col-lg-6">
